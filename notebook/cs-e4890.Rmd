...
def create_cleaned_file(from_name, to_name, cols, drop_duplicates):
    ...
    if drop_duplicates:
        ...
        try:
            # will throw a DataError, when there are no numeric columns present
            # (such as when reading test data)
            data = data.groupby('comment_text').mean().reset_index()
        except pd.core.base.DataError:
            data = data.groupby('comment_text').first().reset_index()
...
train_data = read_cleaned_file('train', 'train_cleaned', relevant_columns)
print_cleanup_summary("train_cleaned")

print("reading test data...")
test_data = read_cleaned_file('test', 'test_cleaned')
print_cleanup_summary("test_cleaned")
```
...
test_data_tensor = torch.tensor(train_tokens, dtype=torch.long).to(device)
...
for epoch in range(4):
    scheduler.step()
    lstm.train()
    for comment, labels in trainloader:
