...
from util import get_individual_words
...
def read_cleaned_file(from_name, to_name, cols, drop_duplicates=True):
...
```{python}
data = read_cleaned_file('train', 'train_cleaned', relevant_columns)
print_cleanup_summary("train_cleaned")
```

```{python}
data.sort_values(by=["target", "jewish"], ascending=False).head(10)
```
...
```{python}
train_x = data["comment_text"]
train_y = data[["target"] + identity_columns]
```
...
tokenizer.fit_on_texts(train_x) # TODO: add test_x

words, _ = get_individual_words(train_x)
...
train_data = torch.tensor(train_tokens, dtype=torch.long).to(device)
train_tags = torch.tensor(train_y.values, dtype=torch.float32).to(device)
```
...
class LSTMNetwork(nn.Module):
    def __init__(self, max_word_len, vocab_size, hidden_size, out_size):
        super(LSTMNetwork, self).__init__()
        
        self.embedding = nn.Embedding(vocab_size, max_word_len)
        self.lstm = nn.LSTM(max_word_len, hidden_size)
        self.linear = nn.Linear(hidden_size, out_size)
        
        self.embedding = nn.Embedding(vocab_size, max_word_len)
        
        self.lstm1 = nn.LSTM(max_word_len, hidden_size, bidirectional=True, batch_first=True)
...
```{python}
lstm = LSTMNetwork(max_word_len, len(tokenizer.word_index) + 1, 128, len(train_y.columns))
...
trainloader = torch.utils.data.DataLoader(
    torch.utils.data.TensorDataset(train_data, train_tags),
    batch_size=512,
    shuffle=True
)
...
for epoch in range(4):
    scheduler.step()
    lstm.train()
    for comment, tags in trainloader:
