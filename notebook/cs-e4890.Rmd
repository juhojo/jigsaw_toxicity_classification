...
```{python}
words, _ = get_individual_words(train_x.append(test_x)) # get individual words from train and test data
...
```{python}
lstm = LSTMNetwork(max_word_len, len(tokenizer.word_index) + 1, 128, len(train_y.columns)).to(device)
...
predictions = pd.DataFrame.from_dict({
    ...
    'prediction': torch.sigmoid(lstm(test_data_tensor).detach().cpu())[:, 0]
})
