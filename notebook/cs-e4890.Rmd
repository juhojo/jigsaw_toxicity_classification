...
scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda epoch: 0.75 ** epoch)
```
...
for epoch in range(epochs):
    ...
    for i, (x, labels) in enumerate(train_loader, 1):
        ...
        if i % log_nth == 0:
            print(total_loss / i, f"{i} / {len(train_loader)} (batches ran / left to run)")
    print(
        f"epoch: {e}/{epochs}",
        f"loss: {total_loss / len(train_loader)}:.4f",
        f"time passed: {time.time() - epoch_start}"
    )
```
